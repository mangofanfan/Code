...
Python中同样存在这样的功能，并且相比于C语言更加简洁优雅。
...
在字符串的单引号或双引号之前添加字母`f`，然后可以在字符串中嵌入变量。以上代码的运行结果为：

```python
Hello, Mango! 你今年已经18岁辣！
```

或者，使用字符串对象的`format`方法也可以实现相同的效果。下面的代码与上面等价。
...
在后面我们编写Python中的函数时，在函数末尾的`return`后面跟上多个用逗号间隔的值是合法的。聪明的你一定猜到了，是函数返回时将多个变量包装成了元组，对吧~
...
在Python中，**字典是无序的数据结构。** Python至多可以确保字典在打印时有序，但作为开发者，你不应该让你的程序依赖于字典中键值对的顺序。
